...
def syllablesInWord(word):
    ...
    if word[-1] == 's':
        ...
        if len(word) >= 4 and word[-4:] in ['tche', 'ysse', 'esse', 'asse', 'ishe', 'ashe', 'ange', 'orce', 'ince', 'ence', 'eeze'] or word[-3:] in ['ace']:
            count += 1
    ...
    if len(word) >= 5 and word[-4:] in ['shed', 'ried', 'lied', 'ssed', 'lked', 'lled', 'wled', 'bled', 'nked', 'cked', 'rmed', 'rked', 'died', 'nged', 'ssed', 'ghed', 'sked', 'gged', 'cied', 'reed', 'ched']:
        count -= 1
